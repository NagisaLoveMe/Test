...
extern void *oa_med_malloc(oa_uint32 size);
extern void oa_med_free(void **ptr);
/**
...
/*Desciption for Error Code
* 0x00000000 Execute with no error
* 0x00000001 Reserved
* 0x00000002 Invalid address (block index, page index)
* 0x00000003 Invalid data length
* 0x00000004 Reserved
* 0x00000005 Write operation fail
* 0x00000006 Erase operation fail
*/

oa_int32 oa_erase_rawdisk_block(oa_uint32 blkIdx);
oa_int32 oa_read_rawdisk_data(oa_uint32 addr, oa_uint8* data_ptr, oa_uint32 len);
oa_int32 oa_write_rawdisk_data(oa_uint32 addr, oa_uint8* data_ptr, oa_uint32 len);
...
*
* DESCRIPTION
*   The function is to init PWM1 clock and div .
*
* PARAMETERS
* clock is oa_pwmclk_13M or oa_pwmclk_32k
* clkdiv is oa_pwmclk_1MHZ,oa_pwmclk_2MHZ,oa_pwmclk_4MHZ,oa_pwmclk_8MHZ
* 
* RETURNS
*  none

*************************************************************************/
extern void oa_pwm_clk_init(oa_pwmclock_sel pwmclk_sel,oa_pwmclock_div pwmclk_div);
...
*
* DESCRIPTION
*   The function is to generate PWM1 output according to the freq .
*
* PARAMETERS
*   freq: frequency in unit of Hz 
*   duty: duty cycle percentage 
* Design Formula:
* clock = 13000000 or 32000
* clkdiv = 1|2|4|8
* cust_freq = 1000000;
* cust_duty = 34;
* pwm_count = ((clock/clkdiv)/cust_freq)-1 =2;
* pwm_thres = (((clock/clkdiv)/cust_freq)*cust_duty/100) =1;
* freq = clock/(clkdiv*(pwm_count+1))
* duty = pwm_thres/(pwm_count+1);
* 
* RETURNS
*  none

*************************************************************************/
extern void oa_pwm_start(oa_uint32 freq, oa_uint8 duty);
...
*
* DESCRIPTION                                                           
*   	This function is to stop PWM1 output.
*
*
* PARAMETERS
*	
...
*  if (NULL == handle)
*  {
*      handle = oa_hw_i2c_open(OA_DCL_I2C_OWNER_1);
*  }
*  </code>
*
*************************************************************************/
extern oa_hw_i2c_handle oa_hw_i2c_open(oa_dcl_i2c_owner flag);
...
/**************************START GPT2 TIME************************************/
/**********************************************************************************/
oa_gpt_handle oa_gpt_open(oa_gpt_dev gpt_dev, oa_dcl_flags dcl_flag);

oa_dcl_status_t oa_gpt_close(oa_gpt_handle handle);

oa_dcl_status_t oa_dcl_hgpt_control(oa_gpt_handle handle,                                                                   
                                                                       oa_hgpt_cmt_t cmd,                                                                         
                                                                       oa_hgpt_ctrl *data);

oa_dcl_status_t oa_dcl_hgpt_register_callback(oa_gpt_handle handle,                                                                                
                                                                                              oa_dcl_event event,                                                                                
                                                                                              oa_pfn_dcl_callback callback);
/*****************************************************************************/
/**************************END GPT2 TIME*************************************/
/*****************************************************************************/
 ...
 *  oa_open_network_led
 * DESCRIPTION
       ForNetwork LED flash Blink control, Default MD251P network led pin19(gpio 0)
       
Refer MD251 module
State        MD251P function    
64ms On/ 800ms Off  MD251P does not find the network
64ms On/ 3000ms Off  MD251P find the network
 ...
 *  Power on GPS module 
 * PARAMETERS
 *  void (* callback)(oa_bool)      
 * RETURNS
 *  
 *****************************************************************************/   
void oa_gps_power_on(void (* callback)(oa_bool));
 ...
 *
 *****************************************************************************/ 
void oa_gps_shutdown(void (* callback)(oa_bool));

void oa_gps_nmeadata_recv_register(oa_uart_rx_cb_fptr fptr);

void oa_bt_spp_receive_data_register(oa_bt_spp_receive_data_fp cb);
 ...
 *   
 * PARAMETERS
 *  void  oa_uint8 *data, data need "\r\n" end mark.
 * RETURNS
 *  
 * EXAMPLE:
 *  oa_bt_spp_write("BT SPP test data!\r\n",oa_strlen("BT SPP test data!\r\n"));
 *******************************************************************************/
oa_uint32 oa_bt_spp_write(oa_uint8 *data, oa_uint16 len);
...
oa_int8 oa_soc_setsockopt(oa_int8 sockId, oa_uint32 option, void *val,oa_uint8  val_size);
